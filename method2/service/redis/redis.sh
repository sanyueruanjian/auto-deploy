...
read -p "请输入redis容器名($ID): " name
read -p "请输入宿主机与redis容器6379端口映射的端口(默认:6379): " port

sed -e "s/REPLACE_NAME/${name}/g" -e "s/REPLACE_PORT/${port:-6379}/g" $CURRENT_DIR/redis.yml >> $PROJECT_DIR/docker-compose.yml
